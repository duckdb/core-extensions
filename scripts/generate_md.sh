...
curl -s https://extensions.duckdb.org/downloads-last-week.json -o build/downloads-last-week.json
...
EXTENSIONS_CSV=$DOCS/core_extensions.csv
...
do
    ...
    echo "warning: DO NOT CHANGE THIS MANUALLY, THIS IS GENERATED BY https://github/duckdb/core-extensions repository, check README there" >> $EXTENSION_README
    ...
    echo "image: '/images/core_extensions/social_preview/preview_extension_"$extension".png'" >> $EXTENSION_README
    cat $EXTENSION_README > $EXTENSION_SCREENSHOT
    echo "layout: core_extension_sql_screenshot" >> $EXTENSION_SCREENSHOT
    echo "---" >> $EXTENSION_SCREENSHOT
    cat layout/screenshot.md >> $EXTENSION_SCREENSHOT


    echo "layout: core_extension_doc" >> $EXTENSION_README
    echo "---" >> $EXTENSION_README
...
rm -f x.db
$1 $DOCS/x.db -markdown -c "SELECT '['||#1||']({% link core_extensions/extensions/'||#1||'.md %})' as Name, '[<span class=github>GitHub</span>](https://github.com/'||#2||')' as GitHub , #4 as Description FROM read_csv('build/docs/core_extensions.csv');" > $DOCS/extensions_list.md.tmp
rm -f x.db
